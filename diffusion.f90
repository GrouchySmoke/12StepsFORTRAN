...
real::dx,dy,dt,lx,ly,cou,cou1,cou2,gamma,t
real::start,end
...
read *,t
write(*,*)"Please enter kinematic viscosity"
read *,gamma
...
!processing the 2D diffusion convection equation
call cpu_time(start)
	do tt=0,tsim
		do i=1,nx
            do j=1,ny
			    utpo(i,j)=u(i,j)+(((gamma*dt)/(dx**2))*(u(i+1,j)-(2*u(i,j))+u(i-1,j)))+(((gamma*dt)/(dy**2))*(u(i,j+1)-(2*u(i,j))+u(i,j-1)))
